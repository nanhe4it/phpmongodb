    ...
    foreach ($this->data['format'] as $format) {
        ...
        <div id="record-<?php echo $format; ?>" style="display: <?php echo $format === 'json' ? 'block' : 'none'; ?>">
            ...
            foreach ($this->data['record'][$format] as $cursor) {
                ...
                echo '&nbsp<a href="javascript:void(0)"  onclick="callAjax(\''.Theme::URL('Collection/EditRecord',array('db'=>$this->db,'collection'=>$this->collection,'id'=>$this->data['record']['document'][0]['_id'],'format'=>$format,'id_type'=>gettype($this->data['record']['document'][0]['_id']))).'\')" class="btn"z">'.I18n::t('EDIT').'</a>';
